...
- **Federation runs WITHIN a single workspace's connections, never across workspaces.** `multi_schema_loader` only loads the bundles for `state.resolved_workspace_id`. Cross-workspace joins are explicitly deferred per `PLAN.md`. Each sub-query is validated by its own dialect's validator before the parallel `asyncio.gather`. Merge pipeline output is capped at `FEDERATION_MAX_ROWS` (default 1000) to defend against cartesian-join blowups.
- **Federation dispatches dialect ONLY at three legal points.** `query_planner` chooses a system prompt (`_SQL_SYSTEM` | `_ES_SYSTEM` | `_MONGO_SYSTEM`). `query_validator` and `federated_executor._run_one` choose a validator (`validate_readonly` | `validate_es_query` | `validate_mongo_query`). `get_engine(source)` constructs the right adapter. Outside these three points, no other module should branch on dialect — when in doubt, push the logic into the engine.
- **Production secret gate.** `main.py::_check_secrets()` fail-fasts when `QM_ENVIRONMENT=production` is set and `JWT_SECRET` is the dev default / shorter than 16 chars, or `QM_MASTER_KEY` is the placeholder. Dev / test get a one-line WARNING instead so unit tests run without ceremony. Don't add other secrets to this gate without updating both call sites in `main.py` AND `backend/.env.example`.
- **SSE error events are sanitized.** `_sanitize_error_for_client` in `api/chat.py` strips SQL fragments, connection strings, and driver internals before yielding the error frame. The full traceback still lands in logs under the same `trace` id. Don't yield raw `str(exc)` from any future SSE handler.
- **Rate limiting is per-token-or-IP.** `app/limiter.py::_key_func` hashes the first 20 chars of the Authorization bearer for a cheap stable key, falls back to client IP for unauthenticated routes. `/chat` is 10/min, `/auth/*` is 5/min. Storage URI defaults to `redis://…/1` so counters never collide with Celery's `db=0`. Decorators MUST place `request: Request` as the first positional argument of the handler (slowapi limitation).
- **Prometheus label cardinality is bounded.** `app/metrics.py` defines four metric singletons (`qm_chat_turns_total`, `qm_query_history_total`, `qm_llm_calls_total`, `qm_chat_duration_seconds`). Labels are small finite enums (`intent`, `dialect`, `status`, `node`, `outcome`). Don't add `user_id` / `session_id` / `sql_text` / connection_id / IP as labels — that's a metric-cardinality blowup waiting to happen.
